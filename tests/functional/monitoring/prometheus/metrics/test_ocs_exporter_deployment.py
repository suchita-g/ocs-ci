...
def test_exporter_pod_running():
    """
    Test ocs-metrics-exporter pod is running in Internal/Provider mode.

    This test runs on both internal and provider clusters. The deployment mode
    is detected at runtime based on the cluster configuration.
...
def test_kube_rbac_proxy_removed():
    ...
    for container in containers:
        ...
        assert "kube-rbac-proxy" not in image.lower(), (
    ...
    try:
        ...
        for endpoint in endpoints:
            ...
            # Should target exporter port directly
            assert "proxy" not in port.lower(), (
...
def test_readiness_endpoint_healthy():
    ...
    assert "/readyz" in probe_path or "/ready" in probe_path, (
