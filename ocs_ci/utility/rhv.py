...
class RHV(object):
    ...
    def get_vm_names(self):
        ...
        """
        return [vm.name for vm in self.get_vms_by_pattern(filter_by_cluster_name=True)]
                ...
                try:
                    ...
                    for status in TimeoutSampler(timeout, 5, self.get_vm_status, vm):
                        if not look_up and status != expc_status:
                            logger.info(
                                f"Waiting for RHV Machine {vm} to {expc_status}"
                                f"Current status is : {status}"
                            )
                            continue
                    ...
                    logger.error(
                        f"RHV VM {vm} is still not {expc_status} after "
                        f"initiating reboot"
                    )
                    raise

    def restart_rhv_vms_by_stop_and_start(self, vm_names, wait=True, force=False):
        """
        ...
        Args:
            ...
            wait (bool): Wait for RHV VMs to start
        """
        self.stop_rhv_vms(vm_names, force=force)
        self.start_rhv_vms(vm_names, wait=wait)
